...
void handle_file_request(int client_fd, const http_request_t *request) {
    ...
    if (strstr(request->url, "..") || strstr(request->url, "//")) {
        build_error_response(client_fd, 403, "Forbidden");
        return;
    }
    ...
    if (stat(file_path, &file_stat) < 0) {
        build_error_response(client_fd, 404, "File Not Found");
        return;
    }
    ...
    if (S_ISDIR(file_stat.st_mode)) {
        ...
        if (stat(index_path, &file_stat) == 0 && S_ISREG(file_stat.st_mode)) {
            serve_file(client_fd, index_path);//显示默认首页
        } else {
            serve_directory_listing(client_fd, file_path, request->url);//显示目录列表
        }
        return;
    }
...
static void serve_file(int client_fd, const char *file_path) {
    ...
    printf("已发送文件：%s的 %ld 字节\n", file_path, sent_bytes);
...
static void serve_directory_listing(int client_fd, const char *dir_path, const char *url) {
    ...
    if (!dir) {
        build_error_response(client_fd, 500, "Cannot Open Directory");
        return;
    }
